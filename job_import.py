...
if __name__ == "__main__":
    ...
    if "guid" in input_data_json:
        logging.info("a guid was supplied to the job")
        logging.info("we are getting a signed url for the given guid")
        ...
        logging.info("the signed url is ", signed_url["url"])
    ...
    logging.info("we are creating a new database named ", NEW_DB_NAME)

    create_db_command = text("create database :db")
    logging.info("This is the db create command: ", create_db_command)

    grant_db_access = text("grant all on database :db to sheepdog with grant option")
    logging.info("This is the db access command: ", grant_db_access)
